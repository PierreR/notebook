...
```
nix-shell  while every dockers inside a node is running with `--dns 172.17.0.1`:footnote[testing.nix -I nixpkgs=https://github.com/NixOS/nixpkgs-channels/archiv.e/nixos-uns the e.tar.gz
```
====

You can foalways rce any sc]ript file to run in a nix-shell as such:
```
